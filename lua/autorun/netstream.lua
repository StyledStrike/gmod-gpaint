...
function net.ReadStream(ply, callback)
	...
	if #queue > 1 then
		timer.Create("NetStreamKeepAlive" .. id, net.Stream.Timeout / 2, 0, function()
			net.Start("NetStreamRequest")
			net.WriteUInt(id, 32)
			net.WriteBit(true)
			if CLIENT then net.SendToServer() else net.Send(ply) end
...
net.Receive("NetStreamRequest", function(_, ply)

	local id = net.ReadUInt(32)
	local stream = net.Stream.WriteStreams[id]
		...
		if not client.finished then
			...
			if keepalive then
				if client.keepalives < net.Stream.MaxKeepalive then
					...
					timer.Adjust("NetStreamWriteTimeout" .. id, net.Stream.Timeout, 1)
				end
			else
...
net.Receive("NetStreamDownload", function(_, ply)
